...
vector<double> a(length), b(length), c(length), e(length), f(length);


void init_vector(vector<double> a) {
    for (int i = 0; i < length; i++)
        a[i] = i;
}

void calculeazaSuma(vector<double> a, vector<double> b, vector<double> c, int start, int end, function<double(double, double)> fct) {
    for (int i = start; i < end; i++)
        c[i] = fct(a[i], b[i]);
}

void
parallel_calculeazaSuma(vector<double> a, vector<double> b, vector<double> c, const int n_threads, function<double(double, double)> fct) {
...
void cycle_sum(vector<double> a, vector<double> b, vector<double> c, int start, int jump, function<double(double, double)> fct) {
    for (int i = start; i < length; i += jump)
        c[i] = fct(a[i], b[i]);
}

void
cicle_calculeazaSuma(vector<double> a, vector<double> b, vector<double> c, const int n_threads, function<double(double, double)> fct) {
    vector<thread> tab_thr(n_threads);
...
bool is_ok(vector<double> a, vector<double> b) {
    for (int i = 0; i < length; i++)
        if (a[i] != b[i])
            return false;
    return true;
