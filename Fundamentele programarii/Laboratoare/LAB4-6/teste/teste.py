...
def ruleaza_teste_service_calatorie():
    ...
    alta_calatorie = creeaza_calatorie(acelasi_id, alta_zi_inceput, alta_luna_inceput, alt_an_inceput, alta_zi_sfarsit, alta_luna_sfarsit, alt_an_sfarsit, alta_destinatie, alt_pret)
    try:
        adauga_calatorie_service(calatorii, acelasi_id, alta_zi_inceput, alta_luna_inceput, alt_an_inceput, alta_zi_sfarsit, alta_luna_sfarsit, alt_an_sfarsit, alta_destinatie, alt_pret)
        assert False
    except ValueError as ve:
        assert (str(ve) == "calatorie invalida!\n")
...
def ruleaza_teste_cautare_calatorii():
    ...
    calatorie = creeaza_calatorie(id_calatorie, zi_inceput, luna_inceput, an_inceput, zi_sfarsit, luna_sfarsit, an_sfarsit, destinatie, pret)
    adauga_calatorie_lista(calatorii, calatorie)
    lista = cautare_datesejur(calatorii, calatorie, 8, 9, 2019, 10, 10, 2022)
    assert(lista == [[2, 15, 12, 2020, 16, 12, 2020, 'Berlin', 20.1], [5, 10, 11, 2021, 16, 12, 2021, 'Franta', 100]])
    destinatie = "Berlin"
    pret=23.555
    lista = cautare_destinatie_pret(calatorii,calatorie,destinatie,pret)
    assert(lista == [[2, 15, 12, 2020, 16, 12, 2020, 'Berlin', 20.1]])
    lista = cautare_datasf(calatorii,calatorie,zi_sfarsit,luna_sfarsit,an_sfarsit)
...
def ruleaza_teste_rapoarte_calatorii():
    ...
    assert(rapoarte_destinatie(calatorii,calatorie,destinatie_data) == 2)
    alta_destinatie_data="Berlin"
    assert(rapoarte_destinatie(calatorii, calatorie, alta_destinatie_data) == 0)
    id_calatorie = 3
    zi_inceput = 10
    luna_inceput = 11
    an_inceput = 2021
    ...
    calatorie = creeaza_calatorie(id_calatorie, zi_inceput, luna_inceput, an_inceput, zi_sfarsit, luna_sfarsit, an_sfarsit, destinatie, pret)
    adauga_calatorie_lista(calatorii, calatorie)
    lista = rapoarte_perioada(calatorii,calatorie,zi_inceput,luna_inceput,an_inceput, zi_sfarsit, luna_sfarsit, an_sfarsit)
    assert(lista == [[5, 10, 11, 2021, 16, 12, 2021, 'Franta', 100], [3, 10, 11, 2021, 16, 12, 2021, 'Berlin', 9999]])
    destinatie="Franta"
    assert(rapoarte_mediepret(calatorii,calatorie,destinatie) == 60.05)
...
def ruleaza_teste_filtrare_calatorii():
    ...
    pret_dat= 25.10
    assert(filtrare_pret_destinatie(calatorii,calatorie,destinatie_data,pret_dat) == [[5, 10, 11, 2021, 16, 12, 2021, 'Franta', 100]])
    luna_data=12
    assert(filtrare_luna(calatorii,calatorie,luna_data) == [[2, 15, 1, 2020, 16, 1, 2020, 'Berlin', 20.1]])
...
def ruleaza_teste_stergere_calatorii():
    ...
    adauga_calatorie_lista(calatorii, calatorie)
    destinatie_data="Franta"
    assert(stergere_destinatie(calatorii,calatorie,destinatie_data) == [[2, 15, 1, 2020, 16, 1, 2020, 'Berlin', 20.1]])
    id_calatorie = 5
    zi_inceput = 10
    luna_inceput = 11
    an_inceput = 2021
    ...
    adauga_calatorie_lista(calatorii, calatorie)
    numar_zile_dat = 5
    assert(stergere_nrzile(calatorii,calatorie,numar_zile_dat)==[[5, 10, 11, 2021, 16, 11, 2021, 'Franta', 100]])
    id_calatorie = 2
    zi_inceput = 15
    luna_inceput = 1
    an_inceput = 2020
