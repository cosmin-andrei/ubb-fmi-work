...
template<typename T>
void VectorDinamic<T>::erase(T elem) {
	IteratorVector<T> it = begin();
	while (it.valid() && *it != elem) {
		++it;
	}
	if (!it.valid()) {
		// daca elementul nu a fost gasit in vector, iesim din functie
		return;
	}

	// mutam toate elementele de dupa elementul gasit cu o pozitie inainte
		// mutam toate elementele de dupa elementul gasit cu o pozitie inainte
	for (IteratorVector<T> j = it; j.valid(); ++j) {
		if (j.poz < lungime - 1) {
			*j = *(j + 1);
		}
	}

	// actualizam lungimea vectorului
	lungime--;

	// verificam daca este nevoie sa realocam memorie
	if (capacitate > INITIAL_CAPACITY && lungime < capacitate / 4) {
		resize();
	}
}
