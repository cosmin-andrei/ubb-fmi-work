...
void FileRepo::load_from_file() {
	...
	while (getline(fin, str)) {
		...
		DisciplinaRepository::store(Disciplina(dis[0], nr, dis[2], dis[3]));
	}
	fin.close();
}
...
void FileRepo::save_to_file() {
	...
	for (auto& it : DisciplinaRepository::getAllDiscipline()) {
		fout << it.getDenumire() << ";" << it.getOre() << ";" << it.getTip() << ";" << it.getProfesor() << "\n";
	}
	fout.close();
}
